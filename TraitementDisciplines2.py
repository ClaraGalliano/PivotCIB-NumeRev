...
#    DocMots = [mot for phrase in docs for mot in re.split('\W+', phrase) if mot not in stopwords ]
    docs2 = set([' '.join(chaine) for chaine in Phrase_En_Mots(docs)])
#    df= pd.DataFrame(docs) # les chaines originales
    df2 = pd.DataFrame(docs2) # la version plus propre
...
with open('HierarchieDisciplineComplet2.json', 'wb') as ficRes:
...
with open('HierarchieDisciplineCompletbis2.json', 'wb') as ficRes:
    ficRes.write(toto.encode('utf8'))

toto = json.dumps(HierarchieJsonSimple)
with open('HierarchieDisciplineSimplifie2.json', 'wb') as ficRes:
    ficRes.write(toto.encode('utf8'))
